...
export default function CategoryCard({ category, index = 0 }) {
  const pastelStyles = [
    { bg: 'bg-[#FCE7F3]', border: 'border-pink-200/70', accent: 'bg-pink-50' }, // Pop (Pink)
    { bg: 'bg-[#E0F2FE]', border: 'border-sky-200/70', accent: 'bg-sky-50' },   // Rock (Sky Blue)
    { bg: 'bg-[#DCFCE7]', border: 'border-emerald-200/70', accent: 'bg-emerald-50' }, // Chill (Mint Green)
    { bg: 'bg-[#FEF3C7]', border: 'border-amber-200/70', accent: 'bg-amber-50' }, // Focus (Cream)
    { bg: 'bg-[#EDE9FE]', border: 'border-purple-200/70', accent: 'bg-purple-50' }, // Podcast (Lavender)
    { bg: 'bg-[#FFEDD5]', border: 'border-orange-200/70', accent: 'bg-orange-50' }, // Urbano (Peach)
    { bg: 'bg-[#E0E7FF]', border: 'border-indigo-200/70', accent: 'bg-indigo-50' }, // Indie (Periwinkle)
    { bg: 'bg-[#CCFBF1]', border: 'border-teal-200/70', accent: 'bg-teal-50' },   // Electrónica (Aqua)
  ];
  ...
  return (
    ...
    >
      {/* Título de la categoría (Negrita, oscuro arriba a la izquierda) */}
      <h3 className="font-black text-lg sm:text-xl text-brand-secondary tracking-tight z-10">
        {category.name}
      </h3>

      {/* Tarjeta diagonal 3D en la esquina inferior derecha */}
